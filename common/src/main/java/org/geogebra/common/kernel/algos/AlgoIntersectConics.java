...
public class AlgoIntersectConics extends AlgoIntersect implements SymbolicParametersBotanaAlgo {
	...
	public final void compute() {
		 ...
		 * System.out.print(permutation[i]+"\t"); } System.out.println(""); for (int
		 * i=0; i<4; i++) { System.out.println(D[i].toString() + "\t" + P[i].toString()
		 * + "\t" + Q[i].toString()); } }
		 */
	...
	private static GeoPointND getPointFrom1on2(GeoConic A, GeoConic B) {
		...
		if (pointsOnConic != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (p.isLabelSet() && p.getIncidenceList() != null && p.getIncidenceList().contains(B)) {
	...
	final private void computeContinuous() {
		...
		// find permutation
		setNearTo(P, isPalive, Q, isQonPath, distTable, pointList, permutation, !isPermutationNeeded,
	 ...
	 */
	final static private void intersectWithDegenerate(GeoConic conic, GeoConic degConic, GeoPoint[] points,
			double eps) {
	 ...
	 */
	final private boolean calcIntersectionPoints(GeoConic conic1, GeoConic conic2, GeoPoint[] points, double eps) {
		/*
		...
		if (absCrossProduct(Amatrix[0], Amatrix[1], Bmatrix[0], Bmatrix[1]) < eps
			...
			/*
			 * //sol[0] = -1.0; // set single line matrix flatDeg[0] = 0.0; flatDeg[1] =
			 * 0.0; flatDeg[3] = 0.0; flatDeg[2] = A.matrix[2] - B.matrix[2]; flatDeg[4] =
			 * A.matrix[4] - B.matrix[4]; flatDeg[5] = A.matrix[5] - B.matrix[5];
			 * 
			 * // classify degenerate conic degConic.setDegenerateMatrixFromArray(flatDeg);
			 * 
			 * // try first conic intersectWithDegenerate(A, degConic, points); if
			 * (testPoints(A, B, points, AbstractKernel.MIN_PRECISION)) return true;
			 * 
			 * // try second conic intersectWithDegenerate(B, degConic, points); if
			 * (testPoints(A, B, points, AbstractKernel.MIN_PRECISION)) return true;
			 */
		...
		eqn[0] = flatA[2] * (flatA[0] * flatA[1] - flatA[3] * flatA[3])
				+ flatA[4] * (2.0 * flatA[3] * flatA[5] - flatA[1] * flatA[4]) - flatA[0] * flatA[5] * flatA[5];
		// x
		...
		eqn[3] = flatB[2] * (flatB[0] * flatB[1] - flatB[3] * flatB[3])
				+ flatB[4] * (2.0 * flatB[3] * flatB[5] - flatB[1] * flatB[4]) - flatB[0] * flatB[5] * flatB[5];
		...
		// if we get this far then we are probably at special case where conics are
		// tangent
	...
	private boolean intersectSpecial(GeoConic c1, GeoConic c2, GeoPoint[] points, double eps) {
		...
		for (int i = 0; i < roots; i++) {
			tempLine.setCoords(1, 0, -eqn[i]);

			// intersect vertical lines with the conics

			// try first conic
			AlgoIntersectLineConic.intersectLineConic(tempLine, c1, points, eps);
			if (testPoints(c1, c2, points, eps)) {
				return true;
			}
	...
	private boolean intersectConicsWithEqualSubmatrixS(GeoConic c1, GeoConic c2, GeoPoint[] points, double eps) {
		...
		} else if (isZero(c1matrix[0]) && isZero(c2matrix[0]) && isZero(c1matrix[3]) && isZero(c2matrix[3])) {
	 ...
	 */
	final public static void distanceTable(GeoPoint[] D, int[] age, GeoPoint[] Q, double[][] table) {
		int i, j;
		...
		/*
		 * for (i=0; i < D.length; i++) { Application.debug("D[" + i + "] = " + D[i] +
		 * "   Q[" + i + "] = " + Q[i]); }
		 * 
		 * // print table for (i=0; i < D.length; i++) { for (j=0; j < Q.length; j++) {
		 * // check if entry is marked as undefined (Q[j]) // set all distances to Q[j]
		 * to max+1 System.out.print(table[i][j] + "\t"); } Application.debug(); }
		 * Application.debug();
		 */
	}
	...
	final static void setNearTo(GeoPoint[] P, boolean[] isPalive, GeoPoint[] Q, boolean[] isQonPath,
			double[][] distTable, PointPairList pointList, int[] permutation, boolean needStrict, double eps) {
		int indexP, indexQ;
		...
		if (!needStrict || (gap > eps * eps)) {
			...
			while (!pointList.isEmpty() && pointList.size() != currentSize) {
				...
				if (pair.isPalive && pair.isQonPath && pointList.getClosestPWithindexQ(pair.indexQ) == pair.indexP
